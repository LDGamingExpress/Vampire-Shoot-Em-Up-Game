...
# "Player", "Enemy", "Ally"
var Damage = 5
var Speed = Globals.PlayerSpeed + randi_range(-5,5) - 50.0
...
func _ready() -> void:
	if randi_range(0,100) > 50:
		...
		XPosWant = -400.0
		SignTimesRotation = -1
	velocity = Vector2(Speed * cos(rotation - (PI/3 * SignTimesRotation)),Speed * sin(rotation - PI/3 * SignTimesRotation))
